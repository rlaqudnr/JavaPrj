...
public class UserController {
	...
	public String delete(@PathVariable Long id, HttpSession session) {
		...
		List<Question> question= questionRepository.findBywriters_id(id);
		List<Answers> answer= answerRepository.findBywriters_id(id);
	for (int i=1; i<=question.size(); i++) {
		//회원이 쓴 게시글 삭제
		questionRepository.deleteBywriters_id(id);
	}
	for (int i=1; i<=answer.size(); i++) {	
		//회원이 쓴 게시글 댓글
		answerRepository.deleteBywriters_id(id);
	}
	//회원탈퇴	
